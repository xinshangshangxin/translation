(function() {
    ...
    if (window.location.href.match(/github.com/)) {
        var r = window.open('', '', '');
        r.opener = null;
        r.document.write(document.documentElement.innerHTML.replace(/(<\/head.*?>)/, '<script type="text/javascript" src="https://xinshangshangxin.chinacloudsites.cn/source/ydtranslate.js"></script>$1'));
        r.document.close();
        return;
    }
    ...
    window.addEventListener("message", function(e) {
        ...
        if (e.data) {
            try {
            	var jsonparse = JSON.parse(e.data);
            	if (jsonparse.s && jsonparse.f && jsonparse.f === 'gbsf') {
            		return;
            	}

                var translatearr = jsonparse.translateResult;
                var htmlarrsrc = [],
                    htmlarrdst = [];
    ...
    function showInfo(e) {
        ...
        setTimeout(function() {
            ...
            if (txtSel) {
                ...
                win.postMessage(txtSel, (protocol.match('file') ? 'http:' : protocol) + '//xinshangshangxin.chinacloudsites.cn/soruce/googletranslate.html');
            }
        }, 20);
    }
    ...
    function addIframe() {
        ...
        var protocol = location.protocol;
        hidehtml.innerHTML = '<iframe style = "display:none" id="youdao_iframe" src="' + (protocol.match('file') ? 'http:' : protocol) + '//xinshangshangxin.chinacloudsites.cn/source/googletranslate.html" ></iframe>';
